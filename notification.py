...
class Notification:
    def __init__(self,knot_id, knot_content, like_number, reknot_number , action_source_pic, action_source, action_type):
    ...
    def select_notifications(self, user_id):
        with dbapi2.connect(database.config) as connection:
            ...
            if len(first_query) > 0:
                ...
                while i < max:
                    if first_query[i][6] == True:
                        action_type = 'liked'
                    else:
                        action_type = 'reknotted'

                    result.append(Notification(first_query[i][0], first_query[i][1], first_query[i][2] , first_query[i][3], first_query[i][4], first_query[i][5], action_type))
                    i = i + 1
